...
\newcommand{\incompleteVignetteText}[0]{\noindent This vignette is not
  complete, since it takes too long to compute, download, or render (owing to
  the lack of bitmaps). Whereever this is the case, the text ``Figure omitted'' 
  is shown. The complete version, dyebiasCompleteVignette.pdf,
  can be found the documentation directory.}

\newcommand{\incompleteVignetteOmission}[0]{\noindent (Text or figure omitted.) }
