...
dye swaps.  \footnote{Whether the columns have been deposited the right way
  ...
  up relative to the input sample and is therefore be visible, in an MA-plot, as a
...
respective reference sample (see the documentation). Having said that, we will
...
been strongly reduced, supporting our assumption of monotonicity in this case.
